...
#parte modificable
def ObtenerPdf():
    nameTemp = 'planillaAguinaldo'
    template = preppy.getModule(f'../templates/{nameTemp}.prep')
    #convert to PDF
    params = {'name': '2021', 'location': 'La Paz'}
    with BytesIO(bytes(template.get(params),'ascii')) as buffer:
        generatePdf(buffer,outputFileName='output.pdf')
